...
struct FilesystemMCPServer {
    ...
    func run() async {
        FileHandle.standardError.write("Filesystem MCP Server with JWT Auth starting...\n".data(using: .utf8)!)
        FileHandle.standardError.write("Protocol version: \(protocolVersion)\n".data(using: .utf8)!)
        FileHandle.standardError.write("Security: JWT + API Key authentication enabled\n".data(using: .utf8)!)
        FileHandle.standardError.write("Auth: Use 'Authorization: Bearer <jwt>' or 'X-API-Key: <key>' headers\n".data(using: .utf8)!)
    ...
    private func handleToolsCall(_ request: JSONRPCRequest) async {
        ...
        // Extract authentication from request metadata (in a real implementation, this would come from HTTP headers)
        let authHeader = params["_auth"]?.value as? [String: AnyCodable]
        let jwt = authHeader?["jwt"]?.value as? String
        let apiKey = authHeader?["api_key"]?.value as? String
        
        // Authenticate request
        let authResult = authManager.authenticate(apiKey: apiKey, jwt: jwt)
        ...
        case .failure(let error):
            sendError(id: request.id, code: error.jsonrpcCode, message: error.localizedDescription)
            return
        }
        ...
        switch toolName {
        case "read_file":
            await handleReadFile(request: request, arguments: arguments, authContext: authContext)
        case "write_file":
            await handleWriteFile(request: request, arguments: arguments, authContext: authContext)
        case "list_directory":
            await handleListDirectory(request: request, arguments: arguments, authContext: authContext)
        case "create_directory":
            await handleCreateDirectory(request: request, arguments: arguments, authContext: authContext)
        case "get_file_info":
            await handleGetFileInfo(request: request, arguments: arguments, authContext: authContext)
        case "search_files":
            await handleSearchFiles(request: request, arguments: arguments, authContext: authContext)
        default:
    ...
    private func handleReadFile(request: JSONRPCRequest, arguments: [String: AnyCodable], authContext: AuthContext) async {
        ...
        guard authManager.authorizeFileAccess(context: authContext, path: sanitizedPath, operation: .read) else {
            sendError(id: request.id, code: -32002, message: "Insufficient permissions to read this file")
            return
        }
    ...
    private func handleWriteFile(request: JSONRPCRequest, arguments: [String: AnyCodable], authContext: AuthContext) async {
        ...
        guard authManager.authorizeFileAccess(context: authContext, path: sanitizedPath, operation: .write(size: contentSize)) else {
            sendError(id: request.id, code: -32002, message: "Insufficient permissions to write to this file")
            return
        }
    ...
    private func handleListDirectory(request: JSONRPCRequest, arguments: [String: AnyCodable], authContext: AuthContext) async {
    ...
    private func handleCreateDirectory(request: JSONRPCRequest, arguments: [String: AnyCodable], authContext: AuthContext) async {
        guard let pathString = arguments["path"]?.value as? String else {
            sendError(id: request.id, code: -32602, message: "Invalid parameters: missing path")
            return
        }
    ...
    private func handleGetFileInfo(request: JSONRPCRequest, arguments: [String: AnyCodable], authContext: AuthContext) async {
        guard let pathString = arguments["path"]?.value as? String else {
            sendError(id: request.id, code: -32602, message: "Invalid parameters: missing path")
            return
        }
    ...
    private func handleSearchFiles(request: JSONRPCRequest, arguments: [String: AnyCodable], authContext: AuthContext) async {
