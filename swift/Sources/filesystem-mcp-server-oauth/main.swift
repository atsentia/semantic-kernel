...
struct FilesystemMCPServer {
    ...
    func run() async {
        FileHandle.standardError.write("Filesystem MCP Server with OAuth 2.1 + PKCE starting...\n".data(using: .utf8)!)
        FileHandle.standardError.write("Protocol version: \(protocolVersion)\n".data(using: .utf8)!)
        FileHandle.standardError.write("Security: Full OAuth 2.1 + PKCE authentication enabled\n".data(using: .utf8)!)
        FileHandle.standardError.write("Auth: Use 'Authorization: Bearer <access_token>' header\n".data(using: .utf8)!)
