...
public final class OpenAIEmbeddingService: EmbeddingGenerationService, @unchecked Sendable {
    ...
    public func generateEmbedding(for text: String) async throws -> [Double] {
        ...
        let req = EmbeddingRequest(input: text, model: model)
        ...
        let resp = try JSONDecoder().decode(EmbeddingResponse.self, from: data)
        lastUsage = resp.usage
        if let total = resp.usage?.total_tokens { metrics.record(.tokensUsed(total)) }
        metrics.record(.callDuration(name: "embedding", seconds: Date().timeIntervalSince(start)))
        guard let first = resp.data.first else { throw AIServiceError.invalidResponse }
